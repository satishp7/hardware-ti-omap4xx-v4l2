...
#define ENTITY_VIDEO_CCDC_OUT_NAME      "OMAP4 ISS CSI2a output"
#define ENTITY_CCDC_NAME                "OMAP4 ISS CSI2a"
#define ENTITY_OV5640_NAME              "ov5640 3-003c"
...
//#define IMG_WIDTH_VGA           1280
//#define IMG_HEIGHT_VGA          720
...
/*		if(version >= KERNEL_VERSION(2,6,37))
		{
			ccdc_fd = open("/dev/v4l-subdev0", O_RDWR);
			if(ccdc_fd == -1) {
...
int V4L2Camera::Configure(int width,int height,int pixelformat,int fps)
{
	...
	if(version >= KERNEL_VERSION(2,6,37))
	{
		LOGD("HACK:%s, kernel version is right", __func__);
