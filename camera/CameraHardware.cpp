...
#define VIDEO_DEVICE_2      "/dev/video1"
...
void CameraHardware::setCameraPath()
{
    //HACK : camera launch based on prop
    ...
    if (cameraId != 0)
        ...
        strcpy(mCameraPath,"/dev/video0");

    LOGD("HACK:setCameraPath:%s", mCameraPath);
    return;
...
status_t CameraHardware::startPreview()
{
    ...
    if(version >= KERNEL_VERSION(2,6,37)) {
        LOGD("HACK:kernel version is greater");
        setCameraPath();
        if (mCamera->Open(mCameraPath) < 0) {
            LOGD("HACK:device open faile");
            return INVALID_OPERATION;
        }
    } else {
        LOGD("HACK:kernel version is less");
    ...
    if (mPreviewThread != 0) {
        LOGD("HACK: Thread is not created");
        return INVALID_OPERATION;
    }
    ...
    //mPreviewWidth = 640;
    //mPreviewHeight =480;
    LOGD("HACK:startPreview width:%d,height:%d",mPreviewWidth,mPreviewHeight);
    if(mPreviewWidth <=0 || mPreviewHeight <=0) {
        LOGE("Preview size is not valid,aborting..Device can not open!!!");
        return INVALID_OPERATION;
    }

    ret = mCamera->Configure(mPreviewWidth,mPreviewHeight,PIXEL_FORMAT,DEFAULT_FRAME_RATE);
    if(ret < 0) {
...
status_t CameraHardware::startRecording()
{
    ...
    //mPreviewWidth = 640;
    //mPreviewHeight = 480;
    LOGD(" start recodring getPreviewSize width:%d,height:%d",mPreviewWidth,mPreviewHeight);
...
int CameraHardware::pictureThread()
{
   ...
   // if (mMsgEnabled & CAMERA_MSG_SHUTTER) {
    ...
    // }
     ...
     //w = 640;
     //h = 480;
     LOGE("Picture Size: Width = %d \t Height = %d", w, h);
    ...
    // height = 480;
     ...
     LOGE("Picture thread : mPreviewWidth = %d \t mPreviewHeight = %d", mPreviewWidth, mPreviewHeight);
     ret = mCamera->Configure(mPreviewWidth,mPreviewHeight,PIXEL_FORMAT,DEFAULT_FRAME_RATE);
     if(ret < 0) {
	     LOGE("Fail to configure camera device");
	     return INVALID_OPERATION;
